...
class Tracker:
    ...
    max_allowed_dist = 100 # allowed dist between 2 detected Roi's to be considered same object
    feature_params = dict(maxCorners = 100,qualityLevel=0.3,minDistance=7,blockSize = 7)
    lk_params = dict(winSize=(15,15), maxLevel = 2, criteria=(cv2.TERM_CRITERIA_COUNT | cv2.TERM_CRITERIA_COUNT,10, 0.03))
    ...
    def track(self, gray , frame_draw):
        p1,st,_ = cv2.calcOpticalFlowPyrLK(self.old_gray, gray, self.p0,None,**self.lk_params) #point tracked, status of those points
        #If no flow, look for new points
        if ((p1 is None) or (len(p1[st==1])<3)):
            self.mode = "Detection"
            self.mask = np.zeros_like(frame_draw)
            self.Reset()
        #If flow, Extract good points
        else:
            #select good points
            good_new = p1[st == 1]
            ...
            frame_draw = frame_draw + self.mask # Display the image with the flow lines
            np.copyto(frame_draw, frame_draw) # copy the data to same address as frame_draw
