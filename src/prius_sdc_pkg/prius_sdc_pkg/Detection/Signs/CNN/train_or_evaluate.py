...
def train_SignsModel(data_dir, IMG_HEIGHT=30, IMG_WIDTH=30,EPOCHS=30, save_model = True, saved_model="saved_model_5_Sign.h5"):
    ...
    #Visualize all the different kinds of classes
    ...
    x_train, y_train, x_test, y_test = train_test_split(np.array(images),labels,test_size=0.4)

    #Model creation
    model = Sequential()

    #First convolutional layer
    model.add(Conv2D(filters=16, kernel_size=3, activation='relu',input_shape=(IMG_WIDTH,IMG_HEIGHT,3)))
...
def EvaluateModelOnImage(model_path,image_path = "",image_label = None):
    ...
    #load dataset
    if image_path == "":
        def_test_image_path = 'prius_sdc_pkg/data/test_images'
        #number of categories
        global NUM_CATEGORIES
        NUM_CATEGORIES = len(os.listdir(def_test_image_path))

        #Visualize all classes
        img_dir = pathlib.Path(def_test_image_path)
        plt.figure(figsize=(14,14))
        for i in range(NUM_CATEGORIES):
...
def main():
    if config.Training_CNN:
        train_SignsModel(os.path.abspath("/prius_sdc_pkg/data/"))
    if Evaluate_Model:
        EvaluateModelOnImage(os.path.abspath("prius_sdc_pkg/data/saved_model_5_Sign.h5"))
