...
def clr_segment(hls,lower_range,upper_range):
    ...
    assert lower_range.shape == (3,)
    assert upper_range.shape == (3,)

    """ lower_range = np.array( [lower_range[0],lower_range[1] ,lower_range[2]] )
    upper_range = np.array( [upper_range[0]    ,255     ,255]) """
...
#Getting the mask of only larger objects
