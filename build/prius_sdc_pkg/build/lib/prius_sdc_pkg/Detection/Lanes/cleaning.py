...
def GetYellowInnerEdge(OuterLanes,MidLane,OuterLane_Points):
    ...
    if Mid_cnts:
        ...
        #Case when there is midlane but no outerlane
        if(not np.any(OuterLanes>0)):
            ...
            #Addressing which side to draw the outerlane when there was no outerlane previously
            DrawRight = False
            ...
            Mid_lowP[1] = MidLane.shape[0] #setting mid_trajectory_lowestPoint_Row to Maxrows
    ...
    #When there is no midlane
    else:
        return OuterLanes, Outer_cnts, Mid_cnts, Offset_correction
    

def ExtendShortLane(MidLane, Mid_cnts, Outer_cnts,OuterLane):
    #Sorting the Mid and Outer Contours based on their rows
    if (Mid_cnts and Outer_cnts):
        ...
        #Connect Midlane to imagebottom by drawing a Vertical line if not already connected
        BottomPoint_Mid = Mid_cnts_Rowsorted[total_no_of_cnts_midlane-1,:]
        if (BottomPoint_Mid[1] < Image_bottom):
            MidLane = cv2.line(MidLane, tuple(BottomPoint_Mid),(BottomPoint_Mid[0],Image_bottom),255,1)
        
        #Connect the outerlane to image bottom 
        #1. Extend the outerlane in the direction of its slope
         # i. Taking the last 20 points of the outerlane to estimate the slope
        ...
        #Estimating the slope
            if(len(RefLast10Points>1)): #2 points needed to estimate a line
                ...
                #Connect outerlane to bottom by drawing a vertical line
