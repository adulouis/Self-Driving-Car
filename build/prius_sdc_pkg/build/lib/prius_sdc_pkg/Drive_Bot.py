...
class Control():
    ...
    def Obey_LeftTurn(self,mode):
        ...
        #Failure to track the left turn sign means start of T-Junction
            ...
            self.Left_turn_iterations = self.Left_turn_iterations + 2
    ...
    def drive(self, Current_State):
        ...
        angle_motor = interp(self.angle,[-45,45],[1,-1])
        if (self.speed!=0):
            speed_motor = interp(self.speed, [30,90], [5,6])
        else:
            speed_motor = 0.0
...
class Car():
    ...
    def display_state(self,frame_disp,angle_of_car,current_speed,tracked_class):
        ...
        angle_of_car  = interp(angle_of_car,[-1,1],[45,-45])
        if (current_speed !=0.0):
            current_speed = interp(current_speed  ,[5  ,   6],[30 ,90])
